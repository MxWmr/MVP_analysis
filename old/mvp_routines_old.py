...
# 
#
# Function read fluo : read fluorometer ascii data for 1 dive 
# 
#
#
################################################################################
...
#
#   input:
#	mvp_dat_name : ASCII MVP .m1 file (mvp_2022xxx.m1)
#
#
#   output:
#
...
def read_mvp_cycle(mvp_dat_name):
...
#
        words = line.split()
        if words[0]=='<END_OF_HEADER>':
#      print('End of header')
            header=False
#
# Now we are out of the header
#
    for i in range(4):
#
# Now we are out of the header
...
#
        line  = fdat.readline()
#
# Stop if this is the end of file
#
        if line=='':
#      print('End of file')  
            break
...
#    print(words)
        ...
        pres =np.append(pres,float(words[0][:-1]))
        salt =np.append(salt ,float(words[5][:-1]))
        cond =np.append(cond ,float(words[4][:-1]))
        temp =np.append(temp ,float(words[3][:-1]))
...
def time_mvp_cycle_up(pres,cond,salt,temp,mvp_tstart,mvp_tend):

#
# Allocate time to each data point
#
# Create a regular time vector (the sampling is supposed to be constant)
#
...
#
    ...
    isurf=np.max(np.where(pres>=pres.min()+0.5))
    pres=pres[ibot:isurf]
    cond=cond[ibot:isurf]
    salt=salt[ibot:isurf]
    temp=temp[ibot:isurf]
    time_cycle=time_cycle[ibot:isurf]

    return pres,cond,salt,temp,time_cycle
...
def time_mvp_cycle_up_bgc(pres,soundvel,cond,temp,do,temp2,suna,fluo,turb,ph,mvp_tstart,mvp_tend):
    ...
    ibot = np.min(np.where(pres == pres.max()))
    isurf = np.max(np.where(pres >= pres.min() + 0.5))
    pres_up = pres[ibot:isurf]
    soundvel_up = soundvel[ibot:isurf]
    cond_up = cond[ibot:isurf]
    temp_up = temp[ibot:isurf]
    do_up = do[ibot:isurf]
    temp2_up = temp2[ibot:isurf]
    suna_up = suna[ibot:isurf] if suna is not None else None
    fluo_up = fluo[ibot:isurf] if fluo is not None else None
    turb_up = turb[ibot:isurf] if turb is not None else None
    ph_up = ph[ibot:isurf] if ph is not None else None
    time_cycle_up = time_cycle[ibot:isurf]
...
def time_mvp_cycle_down(pres,cond,salt,temp,mvp_tstart,mvp_tend):
...
#
# Get only the descending lines
#

    ibot=np.min(np.where(pres==pres.max()))
    isurf=np.min(np.where(pres>=pres.min()+0.5))
    pres=pres[isurf:ibot]
    salt=salt[isurf:ibot]
    cond=cond[isurf:ibot]
    temp=temp[isurf:ibot]
    time_cycle=time_cycle[isurf:ibot]
...
# 
#   input:
#	fluoname     : ASCII Fluorometer file ()
#	Yorig        : time is counted in days since Yorig/1/1 (here 1950/1/1)
#
#   output:
#	fluo_time   : time
...
#
# ################################################################################
def time_mvp_cycle_down_bgc(pres, soundvel, cond, temp, do, temp2, suna, fluo, turb, ph, mvp_tstart, mvp_tend):
    ...
    pres_down = pres[isurf:ibot]
    soundvel_down = soundvel[isurf:ibot]
    cond_down = cond[isurf:ibot]
    temp_down = temp[isurf:ibot]
    do_down = do[isurf:ibot]
    temp2_down = temp2[isurf:ibot]
    suna_down = suna[isurf:ibot] if suna is not None else None
    fluo_down = fluo[isurf:ibot] if fluo is not None else None
    turb_down = turb[isurf:ibot] if turb is not None else None
    ph_down = ph[isurf:ibot] if ph is not None else None

    time_cycle_down = time_cycle[isurf:ibot]
...
#     (Currently, no conversion is applied; values are returned as-is.)
#
#   input:
...
def DO_conversion(do_raw, temp, pres):
    ...
    P_ = A/(1+D*(temp-25)+F*(temp-25)**2) + B/(do_raw*1e-3*(1+D*(temp-25)+F*(temp-25)**2)+C)

    do = G + H*P_
    do = do*(1+E*pres/100)  # Apply pressure correction in MPa
...
def read_mvp_fluo(fluoname,Yorig):

#
# Parameters to transform fluorometer intensity into chlorophyll concentration
#
...
#
    ...
    ffluo=open(fluoname, 'r', encoding = "ISO-8859-1")
    #print('Read '+fluoname)
...
#
# Split this line into words and read the number of lines
#

    words = line.split()
    Nline = np.int32(words[0])
...
#
        ...
        fldate_str=words[0]
        fltime_str=words[1]
        flintensity1=float(words[3])

#    if ( (fllambda1>0.) & (flintensity1>0.) &\
#         (fllambda2>0.) & (flintensity2>0.) ):

        if (flintensity1>=dark_count):

            fluo_dat = fluo_slope * ( flintensity1 - dark_count )

            (mm,dd,yy)=fldate_str.split("/")
            if (yy != 'ff'):
                fldate=date(np.int32(yy)+2000,np.int32(mm),np.int32(dd))
                (hh,mn,sc)=fltime_str.split(":")
                fltime=fldate.toordinal()-date.toordinal(date(Yorig,1,1))+                    (float(hh)+(float(mn)+float(sc)/60.)/60.)/24. 
       
                fluo_time=np.append(fluo_time,fltime)
                fluo_chla=np.append(fluo_chla,fluo_dat)
