...
def gsw_sp_from_c_torch(C, t, p):
    """
    PyTorch compatible version of GSW SP from C calculation
    """
    # Ensure inputs are torch tensors on same device
    device = C.device if isinstance(C, torch.Tensor) else 'cpu'
    ...
    # Broadcast to same shape (PyTorch way)
    C, t, p = torch.broadcast_tensors(C, t, p)
    
    # Constants as torch tensors
    a = torch.tensor([0.0080, -0.1692, 25.3851, 14.0941, -7.0261, 2.7081], 
                     dtype=torch.float32, device=device)
    b = torch.tensor([0.0005, -0.0056, -0.0066, -0.0375, 0.0636, -0.0144], 
                     dtype=torch.float32, device=device)
    c = torch.tensor([0.6766097, 2.00564e-2, 1.104259e-4, -6.9698e-7, 1.0031e-9], 
                     dtype=torch.float32, device=device)
    d = torch.tensor([3.426e-2, 4.464e-4, 4.215e-1, -3.107e-3], 
                     dtype=torch.float32, device=device)
    e = torch.tensor([2.070e-5, -6.370e-10, 3.989e-15], 
                     dtype=torch.float32, device=device)
    k = torch.tensor(0.0162, dtype=torch.float32, device=device)
    
    # Clamp inputs to reasonable ranges to avoid NaN
    C = torch.clamp(C, min=1e-6, max=200.0)
    t = torch.clamp(t, min=-5.0, max=50.0)
    p = torch.clamp(p, min=0.0, max=7000.0)
    
    t68 = t * 1.00024
    ft68 = (t68 - 15) / (1 + k * (t68 - 15))
    
    # Conductivity ratio (avoid division by zero)
    R = 0.023302418791070513 * C  # 1/42.9140
    
    # rt_lc calculation
    rt_lc = c[0] + (c[1] + (c[2] + (c[3] + c[4] * t68) * t68) * t68) * t68
    
    # Rp calculation  
    denominator = 1 + d[0] * t68 + d[1] * t68**2 + (d[2] + d[3] * t68) * R
    denominator = torch.clamp(denominator, min=1e-10)  # Avoid division by zero
    
    Rp = 1 + (p * (e[0] + e[1] * p + e[2] * p**2)) / denominator
    
    # Rt calculation
    rt_lc = torch.clamp(rt_lc, min=1e-10)  # Avoid division by zero
    Rt = R / (Rp * rt_lc)
    
    # Clamp Rt to positive values (PyTorch way, no masked arrays)
    Rt = torch.clamp(Rt, min=1e-10)
    Rtx = torch.sqrt(Rt)
    
    # Main PSS-78 calculation
    SP = (a[0] + (a[1] + (a[2] + (a[3] + (a[4] + a[5] * Rtx) * Rtx) * Rtx) * Rtx) * Rtx + 
          ft68 * (b[0] + (b[1] + (b[2] + (b[3] + (b[4] + b[5] * Rtx) * Rtx) * Rtx) * Rtx) * Rtx))
    
    # For low salinity correction (simplified - avoid complex indexing)
    # Apply correction only where SP < 2
    low_sal_mask = SP < 2.0
    if torch.any(low_sal_mask):
        # Simplified Hill correction (avoid complex indexing for gradient compatibility)
        x = 400 * Rt
        sqrty = 10 * Rtx
        part1 = 1 + x * (1.5 + x)
        ...
        part1 = torch.clamp(part1, min=1e-10)
        part2 = torch.clamp(part2, min=1e-10)
        
        SP_Hill_raw = SP - a[0] / part1 - b[0] * ft68 / part2
        
        # Simple Hill ratio approximation (constant for gradient stability)
        Hill_ratio = 2.0  # Simplified
        SP_corrected = Hill_ratio * SP_Hill_raw
        
        # Apply correction only where needed
        SP = torch.where(low_sal_mask, SP_corrected, SP)
    
    # Ensure positive salinity
    SP = torch.clamp(SP, min=0.0, max=50.0)
...
def gsw_sp_from_c_original(C, t, p):
    """
...
# Salinity should be >2 or Hill-based correction is applied and is currently not compatible with torch autograd
